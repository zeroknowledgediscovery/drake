...
\]
grows rapidly with $m$. This immediately implies a notion of typicality: at a given radius, most outcomes resemble typical draws from the mutation kernel, and only a small fraction are atypical relative to that kernel.
...
We have shown that Drake's rule follows from two-part coding optimality and typical-case bounds on randomness deficiency under mutation constraints. Mutation induces a rapidly growing indexing cost through the size of the Hamming sphere, and only constant-radius mutations can produce mutation-atypical outcomes with non-negligible probability. Combining this scaling with an independent per-site mutation model and maximizing the resulting expected discovery rate yields the observed inverse scaling of mutation rate with genome length, equivalently, an $O(1)$ mutation rate per genome per generation.



%% \section*{Discussion}

%% This work provides a mechanism-independent route to Drake’s rule from the standpoint of variation supply. Under a minimal model in which mutation acts as a local stochastic perturbation on a discrete sequence space, we derived the typical probability that an $m$-mutant is statistically exceptional relative to the mutation neighborhood. Combining this typical-case rarity with an independent per-site mutation model shows that the expected rate at which mutation supplies such exceptional variants is maximized when the expected number of mutations per genome per generation is $O(1)$. In the per-site parametrization, this yields the inverse-length scaling $\mu^\star=\Theta(1/n)$.

%% Our notion of discovery is defined relative to the mutation-induced model. We do not claim that the Hamming sphere is the globally optimal statistical model for an offspring, only that it is the model naturally induced by the mutation mechanism. Since mutation is the sole upstream source of variation, selection can only act on what mutation supplies. In particular, sustained accumulation of biological organization requires a nonzero supply of variants that are atypical relative to mutation alone. If offspring were always typical draws from the mutation kernel, then over many generations the population would be driven toward the typical set of that kernel, and previously accumulated structure would be eroded. Positive randomness deficiency provides a quantitative, mechanism-independent way to express this minimal sense in which a variant is not merely ``background'' mutational noise.

%% The inverse-length scaling is robust to the choice of novelty margin $\Delta$, provided $\Delta$ does not grow with $n$ fast enough to dominate the one-step indexing cost $c(1)=\Theta(\log n)$. In particular, any fixed $\Delta$ or $\Delta=o(\log n)$ yields the same $\mu^\star=\Theta(1/n)$ scaling.
...
%% This framing connects naturally to classical biology concepts. Population-genetic error-threshold arguments, such as Eigen’s, constrain mutation from above by requiring heritable information to remain stable under copying errors~\cite{Eigen1971}. Our result is different in kind: it identifies an \emph{optimal} scaling for the upstream supply of rare variants under blind local perturbations. The fact that both viewpoints emphasize the $1/n$ regime suggests a convergence of two pressures, fidelity constraints that prevent loss of existing information and supply constraints that prevent evolutionary stasis. In this sense, operating near one mutation per genome per generation can be interpreted as a compromise between maintaining inherited structure and generating a steady trickle of atypical variants on which selection can act.

%% \subsection*{Beyond biological specificity}

%% Although motivated by biological evolution, the argument presented here is not inherently biological. No assumptions are made about molecular replication, detailed population structure, or selection dynamics beyond the minimal requirement that selection acts on variants supplied by mutation. The derivation depends only on three ingredients: a discrete configuration space, a local stochastic perturbation mechanism, and the requirement that adaptive progress requires access to variants that are atypical relative to the perturbation mechanism itself.

%% In this sense, Drake’s rule emerges as a special case of a more general principle governing exploration under constrained generative processes. Any system in which novel structure must be discovered through local random perturbations, whether genetic, cultural, technological, or algorithmic, faces an analogous tradeoff. Perturbations that are too small fail to generate exceptionality at an appreciable rate, while perturbations that are too large destroy exploitable structure. The resulting optimum selects a regime in which the expected perturbation size is $O(1)$ relative to system dimension.

%% This perspective suggests that the inverse scaling of mutation rate with genome length is not an idiosyncratic feature of biology, but a manifestation of a general information-theoretic constraint. Biological evolution realizes this constraint through molecular mutation and selection, but the same logic applies to any adaptive process operating under blind local exploration. In this sense, Drake’s rule reflects a law of structure discovery under uncertainty, rather than a contingent property of life as we know it.
...
%% \subsection*{High-dimensional scaling and an intensive ``mutation temperature''}

%% Although the biological statement of Drake's rule is that the per-site mutation rate scales as $\mu \asymp 1/n$, the more intrinsic scaling variable in our analysis is the per-genome mutation intensity
%% \[
%% T \;:=\; n\mu \;=\; \mathbb{E}[M],\qquad M\sim{\rm Binomial}(n,\mu),
%% \]
%% which plays the role of an \emph{intensive} ``temperature'' controlling the typical perturbation size. The optimization result $\mu^\star=(1+o(1))/n$ is equivalently the statement that the discovery functional is maximized at a constant temperature $T^\star=\Theta(1)$.

%% This is consistent with the geometry of high-dimensional discrete spaces. For $x\in[q]^n$, the number of microstates at Hamming distance $m$ is
%% \[
%% |H_m(x)|=\binom{n}{m}(q-1)^m,
%% \qquad
%% S(m):=\log |H_m(x)|=\log\binom{n}{m}+m\log(q-1),
%% \]
%% so $S(m)$ is a Boltzmann-type entropy $S=\log\Omega$ for the macrovariable $m$. In the large-$n$ limit, with $\rho=m/n\in(0,1)$, Stirling's approximation gives
%% \[
%% \frac{1}{n}S(\rho n)
%% =
%% H(\rho)+\rho\log(q-1)+o(1),
%% \]
%% where $H(\rho)=-\rho\log\rho-(1-\rho)\log(1-\rho)$ is the binary entropy. Thus the number of mutation-accessible microstates grows exponentially in $n$ once $\rho$ is bounded away from $0$, reflecting a standard concentration-of-measure phenomenon: for a uniformly random $Y\sim{\rm Unif}([q]^n)$, the distance $d_H(X,Y)$ concentrates sharply around $(1-1/q)n$.

%% This geometry explains why extensive perturbations destroy structure. If $\mu$ is constant (so $T=n\mu=\Theta(n)$), then $M$ concentrates around $\Theta(n)$, and the offspring distribution places its mass on exponentially large shells $|H_M(x)|\approx \exp\{\Theta(n)\}$. In that regime, mutation behaves like an almost-unstructured resampling mechanism: exceptional outcomes relative to mutation alone are exponentially rare because their baseline probability must compete with an exponentially growing indexing entropy $S(M)$. Conversely, if $\mu\ll 1/n$ (so $T\to 0$), then the probability of any change vanishes,
%% \[
%% \Pr(M\ge 1)=1-(1-\mu)^n = n\mu + o(n\mu)=T+o(T),
%% \]
%% and exploration is starved.

%% Our analysis makes this tradeoff quantitative for structure discovery. Under the novelty criterion defined by deficiency threshold $\Delta$, the typical-case bound implies that the probability that an $m$-mutant is mutation-atypical scales as $\Theta(2^{-\Delta}/|H_m(x)|)$ up to polylogarithmic factors, so increasing $m$ rapidly suppresses discovery through the entropy term $S(m)=\log|H_m|$. When combined with the binomial mutation model and expanded for $\mu\ll 1$, the expected discovery rate takes the form
%% \[
%% \Phi(\mu)\;\asymp\;C\,2^{-\Delta}\,\frac{\mu}{q-1}\,e^{-n\mu}
%% \;=\;
%% C\,2^{-\Delta}\,\frac{T}{n(q-1)}\,e^{-T},
%% \]
%% with $C$ absorbing polylogarithmic terms. The factor $T$ captures the vanishing probability of any perturbation when $T\to 0$, while the factor $e^{-T}$ captures the rapid loss of structured discovery as typical displacements grow. Maximizing $T e^{-T}$ yields $T^\star=1$, i.e.\ an optimal intensive temperature and the inverse-length scaling $\mu^\star=(1+o(1))/n$.

%% In this sense, the appearance of a constant optimal temperature is not surprising from the viewpoint of high-dimensional concentration: the only nondegenerate exploration regime is one in which typical perturbations remain $O(1)$ as $n\to\infty$. What is striking empirically is that biological systems appear to operate in precisely this intensive regime, despite enormous diversity of replication biochemistry and selection environments.
...
\subsection*{Sequence Space, Mutation Geometry and Two-Part Codes}
...
\cgather{
H_m(x)=\{y\in[q]^n : d_{\Hamming}(x,y)=m\}.
...
%\begin{lemma}[Two-part code bound]
If $x\in S$ and $S$ is a finite set, then the two-part code bound is as follows:
\cgather{
\K(x) \le \K(S)+\log|S|+O(1),
\qquad
\K(x\mid S)\le \log|S|+O(1).%
}
which expresses  optimality condition of two-part codes~\cite{LiVitanyi2019,GacsTrompVitanyi2001}: first describe the model $S$, then describe the index of $x$ within $S$.
%
Next we propose the mutation process may be conceptualized as producing a  restricted model class. To see this, note that for fixed parent $x$ and mutation radius $m$, the Hamming sphere $H_m(x)$ defines the model class accessible by mutation. Crucially,
$\K(H_m(x)\mid x,m)=O(1)$, 
which implies $
\K(H_m(x))=\K(x)+O(\log n)$,
...
\cgather{
c(m)=\log|H_m(x)|=\log\binom{n}{m}+m\log(q-1).
}%
...
We have the following results on upper and typical bounds on discovery probability.
%\subsection*{Discovery Probability: Upper and Typical Bounds}
%
...
le
O\left(\frac{2^{-\Delta}}{|H_m(x)|}\right).
}
\end{theorem}
...
\cgather{
\pi_{x,m}:\{1,\dots,|H_m(x)|\}\to H_m(x)
}%
such that given $(x,m,i)$ one can compute $y=\pi_{x,m}(i)$. Hence
\cgather{
\K(y\mid x,m)\le \log|H_m(x)|+O(1)
}%
for all $y\in H_m(x)$. For the lower tail, define
\cgather{
A_\Delta=\{y\in H_m(x):\K(y\mid x,m)\le \log|H_m(x)|-\Delta\}.
}%
Each $y\in A_\Delta$ has a prefix-free description of length at most
$\log|H_m(x)|-\Delta$, so $|A_\Delta|\le |H_m(x)|2^{-\Delta+O(1)}$.
Thus
\cgather{
\Pr[y\in A_\Delta]\le 2^{-\Delta+O(1)}.
}%
and since $x$ is incompressible, the two-part code through $H_m(x)$ is optimal up to $O(\log n)$ terms. Therefore
\cgather{
\Delta_y(H_m(x))
=
\log|H_m(x)|-\K(y\mid x,m)\pm O(\log n),
}%
which yields the stated asymptotic bound.
\end{IEEEproof}
%
\begin{cor}[Expected discovery under binomial mutation]
Let $X\sim \mathrm{Unif}([q]^n)$ be the parent sequence.
Let $M\sim \mathrm{Binomial}(n,\mu)$ denote the number of mutated sites,
and conditional on $(X,M=m)$ let
$Y\sim \mathrm{Unif}(H_m(X))$. Define the expected discovery probability
\cgather{
\Phi(\mu)
:=
...
\Pr\!\left[K(X)\le n\log q - t\right]\le q^{-t}
\]
implies that $X$ satisfies the incompressibility condition of Lemma~2
with overwhelming probability.
On this event, Lemma~2 yields
\[
\Pr_{y\sim H_m(X)}
\!\left[\Delta_y(H_m(X)) \ge \Delta \right]
\;\asymp\;
\frac{2^{-\Delta}}{|H_m(X)|}.
\]
The contribution of the exceptional set
$\{X: K(X)\le n\log q - t\}$ is at most $q^{-t}$ and is negligible
for $t=\omega(1)$.
Substituting the tight bound into the definition of $\Phi(\mu)$
gives the stated expression.
\end{IEEEproof}
%
Note, that upon substituting the binomial mutation model and the
sphere cardinality $|H_m(X)|=\binom{n}{m}(q-1)^m$,
the combinatorial factor $\binom{n}{m}$ appearing in
$\Pr(M=m)$ cancels with the identical factor in $|H_m(X)|$.
Thus the multiplicity of mutation-location choices does
not amplify discovery probability: it is already accounted
for by the mutation process itself. What remains is the
symbol-choice entropy $(q-1)^m$ and the exponential
suppression arising from the rarity of exceptional strings
within each mutation-accessible model. Now, we can state the 
small-$\mu$ asymptotics of discovery rate as follows:
%% \subsection*{Optimization and Drake’s Rule}
%
\begin{lem}[Small-$\mu$ asymptotics of discovery rate]
For $\mu=o(1)$,
\cgather{
...
\frac{\mu}{q-1}+O(\mu^2),
\qquad
(1-\mu)^n=e^{-n\mu+O(n\mu^2)}.
}%
Consequently,
\cgather{
\Phi(\mu)\;\asymp\;
C\,2^{-\Delta}\,\frac{\mu}{q-1}\,e^{-n\mu+O(n\mu^2)},
}where $C$ absorbs polylogarithmic factors.
\end{lem}
%
\begin{IEEEproof}
The first expansion is the truncated geometric series.
%% The second follows from $\log(1-\mu)=-\mu+O(\mu^2)$, hence
%% $n\log(1-\mu)=-n\mu+O(n\mu^2)$.
Substituting $n\log(1-\mu)=-n\mu+O(n\mu^2)$ into the expression for $\Phi(\mu)$ gives the claim.
\end{IEEEproof}

\begin{theorem}[Optimal mutation rate]
In the regime $\mu=o(1)$ with $n\mu^2=o(1)$, the leading term
$\mu e^{-n\mu}$ is maximized at $n\mu=1$. Consequently,
\cgather{
\mu^\star=\frac{1+o(1)}{n}.
}
\end{theorem}

\begin{corollary}[Drake’s rule]
Optimizing information-theoretic structure discovery under random mutation
yields an inverse scaling of per-site mutation rate with genome length.
\end{corollary}
