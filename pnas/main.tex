...
If mutation predominantly produces outcomes that are typical under its own kernel, then repeated mutation pushes lineages toward the kernel's typical set, and accumulated organization is continually eroded into mutational background; we descend into noise (See Fig.\ref{figsim}b). Any sustained adaptive process therefore requires a nonzero supply of outcomes that are atypical relative to mutation alone, since these are the only candidates that can be preferentially retained rather than washed out. Thus, we are not proposing any explicit evolutionary objective function; we are analyzing  structural constraints on variation supply induced by local blind search.
...
%Importantly, we do not assume a Poisson mutation process a priori. The simulation uses the exact Binomial model arising from independent per-site substitutions. The exponential factor appearing below is the standard small-$\mu$ approximation $(1-\mu)^n \approx e^{-n\mu}$ obtained when $T=n\mu$ is held fixed and $n\mu^2 \to 0$.


\begin{figure}[!ht]
\includegraphics[width=.5\textwidth]{Figures/External/plotfig}

\vspace{-5pt}

%% \caption{
%% \textbf{Scaling and collapse of the discovery rate under independent per-site mutation.}
%% \textbf{a.} Few genome compression values over time (GISAID) illustrating empirical boundedness of sequence complexity over evolution, and the general lack of compressibility of genomes (the maximum entropy rate for 4 letter alphabet is obviously 2 bits/base, achieved by perfectly incompressible sequences). Shaded bands indicate 95\% confidence intervals over replicate lineages. \textbf{b.}  Toy evolution on $[4]^n$ under mutation alone versus mutation with a viability filter that accepts offspring only if they remain within a fixed Hamming distance of a structured template. The plotted statistic is a reference-free compression proxy (compressed bits per symbol using zlib) along replicate lineages; mutation alone drives trajectories toward a max-entropy regime, while the viability filter keeps trajectories bounded away from that regime.
%% \textbf{c.} The expected discovery rate $\Phi(\mu)$ as a function of the per-site mutation rate $\mu$ for increasing genome lengths $n$ (theory, with Monte Carlo estimates shown as points where available). The maximizer shifts to smaller $\mu$ as $n$ increases, consistent with the inverse-length scaling $\mu^\star \propto 1/n$.
%% \textbf{d.} The same discovery rate plotted against the per-genome mutation intensity $T = n\mu = \mathbb{E}[M]$. Curves for different $n$ collapse onto a common profile with a maximum near $T \approx 1$; the dashed guide curve shows the predicted $T e^{-T}$ dependence up to a multiplicative constant. This collapse demonstrates that the optimal regime corresponds to a constant $O(1)$ number of mutations per genome per generation.
%% }\label{figsim}

\caption{
\textbf{Scaling and collapse of the discovery rate under independent per-site mutation.}
\textbf{a.} Genome compression values over time (viral genomes from GISAID; \textit{E.~coli} from NCBI; see SI) illustrating empirical boundedness of sequence complexity over evolution and the near-incompressibility of genomes under a reference-free compression proxy. The Shannon ceiling for a four-letter alphabet is $2$ bits/base, achieved by perfectly incompressible sequences. Shaded bands indicate 95\% confidence intervals across genomes within each year.
\textbf{b.} Toy evolution on $[4]^n$ under mutation alone versus mutation with a viability filter that accepts offspring only if they remain within a fixed Hamming distance of a structured template. The plotted statistic is a reference-free compression proxy (compressed bits per symbol using \texttt{zlib}) along replicate lineages; mutation alone drives trajectories toward a max-entropy regime, while the viability filter maintains bounded complexity.
...
%% In the left panel, $\Phi(\mu)$ plotted against the per-site rate $\mu$ exhibits a single interior maximum whose location shifts left as $n$ increases, consistent with $\mu^\star\propto 1/n$. In the right panel, plotting the same discovery rate against $T=n\mu$ collapses curves for different $n$ onto a common profile with a maximum near $T\approx 1$, matching the predicted $T e^{-T}$ dependence up to a multiplicative constant.
