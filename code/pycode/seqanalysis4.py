...
NCBI Datasets mode:
  The NCBI download root is expected to contain:
    data_summary.tsv
...
def discover_ncbi_fna_files(ncbi_root: str) -> List[str]:
    """
    Find genomic .fna files under the NCBI root.
    """
...
def main():
    ...
    if args.ncbi:
        ...
        for p in raw_paths:
            if os.path.isdir(p):
                # If a directory is provided in non-NCBI mode, include common FASTA extensions
    ...
    with tqdm(total=total_records, desc="Processing FASTA records", unit="seq") as pbar:
        for path in fpaths:
            ...
            if args.ncbi:
                ...
                if root is None:
                    # should not happen since we discovered from roots
                    pbar.update(count_fasta_records(path))
                    continue
                ...
                rows.append(
                    {
                        ...
                        "ncbi_accession": ncbi_accession if args.ncbi else np.nan,
                        "ncbi_submission_date": ncbi_submission_date if args.ncbi else np.nan,
                    }
                )
