...
VALID_DNA = set(list("ACGTURYSWKMBDHVN"))
...
def _process_fasta_file_worker(args):
    ...
    if ncbi_mode and not ncbi_per_record:
        ...
        header_out = hdr0 if hdr0 is not None else os.path.basename(path)
        if n_contigs > 1:
            header_out = f"{header_out} ;(+{n_contigs-1} contigs)"
    ...
    for rec in iter_fasta(path):
        year = forced_year if ncbi_mode else year_from_header(rec.header, min_year=min_year, max_year=max_year)
        if year is None:
            continue
...
def estimate_overhead_fast_random(
    lengths: List[int],
    level: int,
    k: int,
    seed: int,
    ...
    """
    Fast overhead model for random A/C/G/T ASCII strings:
      C_rand_bits(n) ≈ 8n + const
      O_hat(n) = C_rand_bits(n) - 2n ≈ 6n + const

    We estimate const from k replicates at length n0:
      const_hat = median(C_rand_bits(n0) - 8n0)
    then return overhead_map[n] = max(0, 6n + const_hat).
    """
    uniq = sorted(set(lengths))
    if not uniq:
        return {}

    kk = int(max(1, min(k, 10)))  # keep this small; const stabilizes quickly
    ...
    const_hat = float(np.median(c_bits - 8.0 * n0))
    overhead = {n: max(0.0, 6.0 * float(n) + const_hat) for n in uniq}
    return overhead


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("inputs", nargs="+", help="FASTA paths/globs, or NCBI roots/paths when --ncbi.")

    ap.add_argument("--out_dir", default=".", help="Directory for outputs.")
    ap.add_argument("--tag", default=None, help="Output prefix tag.")
    ...
    ap.add_argument("--ncbi_per_record", action="store_true")

    ap.add_argument("--numcores", type=int, default=10)

    ap.add_argument("--overhead_bin", type=int, default=0)

    ap.add_argument(
        "--overhead_fast_random",
        action="store_true",
        default=True,
        help="Use fast random-control overhead model (recommended, especially for long bacterial genomes).",
    )
    ap.add_argument(
        "--overhead_n0",
        type=int,
        default=200000,
        help="Random-control length used to estimate constant in fast overhead model.",
    )
    ...
    if not args.no_overhead_correction:
        ...
        if args.overhead_fast_random:
            overhead_map = estimate_overhead_fast_random(
            ...
            raise SystemExit("Non-fast overhead model removed in this speed-focused version. Use --overhead_fast_random (default).")
